...
const MyTable1 = () => {
    ...
    return (
        <div className="container mt-4">
            ...
            <table style={{ marginTop: "0px" }} className="custom-table">
                <tbody>
                    ...
                    {!isNaN(vrNo) && (inputValues.length>0 && !isNaN(inputValues[0].vr_no)||I==true) && inputValues.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>{renderInputField(item.item_code, e => handleInputChange(index, 'item_code', e.target.value), index < displayCount || Save,"250px","text")}</td>
                            <td>{renderInputField(item.item_name, e => handleInputChange(index, 'item_name', e.target.value), index < displayCount || Save,"400px","text")}</td>
                            <td>{renderInputField(item.qty, e => handleInputChange(index, 'qty', e.target.value), index < displayCount || Save,"200px","number")}</td>
                            <td>{renderInputField(item.rate, e => handleInputChange(index, 'rate', e.target.value), index < displayCount || Save,"200px","number")}</td>
                            <td>
                                <input
                                    style={{width:"200px"}}
                                    type="text"
