...
import { setError, setI, setJsonArray, setSave, setTotal } from './jsonSlice';
    ...
    useEffect(() => {
        if (Save) {
            ...
            inputValues.map((item)=>{
                const hasEmptyString = Object.values(item).some(value => value === "");
                if(hasEmptyString){
    ...
    return (
        <div className="container mt-4">
            ...
            <div style={{ display: "inline-flex", alignItems: "center", height: "40px", width: "100%", color: "white", background: "#330", marginBottom: "0px" }}>
                <div style={{position:'absolute',left:"4%"}}>Sr No</div>
                <div style={{position:'absolute',left:"13%"}}>Item Code</div>
                <div style={{position:'absolute',left:"30%"}}>Item Name</div>
                <div style={{position:'absolute',left:"46%"}}>Qty</div>
                <div style={{position:'absolute',left:"59%"}}>Rate</div>
                <div style={{position:'absolute',left:"73%"}}>Amount</div>
            </div>
